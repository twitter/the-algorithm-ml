...
Note that not every request will have every feature available due to user settings or other constraints and there may be some differences based on different experiments / changes.

## Aggregate Features
Twitter's aggregate features comprise the bulk of Twitter's feature count and are generated by maintaining rolling aggregations of feature values within a specific scope within a specific time window. We compute aggregates over the long-term (50 days) and short-term ("real-time" - under 3 days and typically 30 mins).
...
<li> The raw feature group names are often verbose and are simplified in the below presentation. In particular the word "pair" which just denotes that a feature cross is happening often appears in the raw feature group names.
</ul>
<li> <b>Engagement Scope</b> is the subset of tweets within the Aggregation Scope will be aggregated over. Typically this is the name of an output engagement, like <code>recap.engagement.is_favorited</code>. In that case, we only aggregate over tweets which are also Liked.
...
What this means is that this feature aggregates
<ol>
<li> (Over every user),
...
These features aggregate counts of some user's interaction with other engagers of tweets that the user interacts with.
...
For all tweets that a user favs, accumulate a running count over 50 days of the number of engagement events for every other user who has engaged with the tweet, where engagement is defined as fav or reply. We now have a list of engagement counts for many other users that have engaged with the tweets that the user has faved, and we take the top count as the feature value.  
...
These features aggregate counts of some user's interaction with tweets that mention other users.
...
Here <code>sparse_sum</code> means we sum the aggregate values over all mentioned users, <code>sparse_top1</code> means we take the max of the aggregate values for the mentioned authors, <code>sparse_top1</code> means we take second-highest of the aggregate values for the mentioned authors, and so on.
...
<br>
User-author interaction features. Similar to realgraph but more realtime. <br>
<table>
<tr>
<td>
<code>
...
<br>
We have two embeddings trained on the user-user follow graph, one representing who is likely to follow a user and the other representing who a user is likely to follow. Each embedding is 200-dimensional. Training was accumulated over data throughout 2022-23.
</details>

<details>
<summary><b><code>Twhin Engagement Embeddings</code></b></summary>
<br>
We have one embedding trained on the user-tweet engagement graph, represeting users based on the tweets they are likely to engage with. This embedding is 200 dimensional. Training was accumulated over data throughout 2022-23.
